...
class MultiLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32),num_conv_relu=2,num_con_relu_p=1, num_filters=32, filter_size=5,
    ...
    self.reg = reg
    self.dtype = dtype
    self.num_conv=num_conv_relu*num_con_relu_p
    self.cache={}
    ...
    count=0
    for i in range(num_con_relu_p):
        for j in range(num_conv_relu):
            count+=1
            W_name='W'+str(count)
            b_name='b'+str(count)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1,self.num_conv+1):
        ...
        b_name='b'+str(i)
        chache_name='c'+str(i)
        if i%self.num_conv_relu==0:
            scores,cache=conv_relu_pool_forward(scores,self.params[W_name],self.params[b_name],conv_param,pool_param)
            # print i, len(cache)
        else:
    ...
    for i in range(self.num_conv,0,-1):
        ...
        loss+= 0.5*self.reg*np.sum(self.params[W_name]**2)   
        if i%self.num_conv_relu==0:
            dscores,grads[W_name],grads[b_name]=conv_relu_pool_backward(dscores,self.cache[cache_name])
        else:
            dscores,grads[W_name],grads[b_name]=conv_relu_backward(dscores,self.cache[cache_name])
        grads[W_name]+=self.reg*self.params[W_name] 
